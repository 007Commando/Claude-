...
export interface PrimewellLeadsResult {
  connected: boolean;
  error?: string;
  totalLeads: number;
  newLeads7d: number;
  ...
  contacts: PrimewellContact[];
  contactsTruncated: boolean;
}

const empty: PrimewellLeadsResult = {
  connected: false,
  totalLeads: 0,
  newLeads7d: 0,
  newLeads30d: 0,
...
export async function getPrimewellLeads(): Promise<PrimewellLeadsResult> {
  ...
  if (!token || !locationId) {
    return {
      ...empty,
      error: "GHL_PRIMEWELL_PRIVATE_INTEGRATION_TOKEN or GHL_PRIMEWELL_LOCATION_ID is not set",
    };
  }
  ...
  while (contacts.length < MAX_CONTACTS) {
    ...
    try {
      ...
      if (contacts.length === 0) {
        return { ...empty, error: err instanceof Error ? err.message : "Failed to reach PrimeWell's GHL" };
      }
