...
export interface FacebookLeadsResult {
  connected: boolean;
  error?: string;
  totalLeads: number;
  newLeads7d: number;
...
const empty: FacebookLeadsResult = {
  connected: false,
  totalLeads: 0,
  newLeads7d: 0,
  newLeads30d: 0,
...
export async function getFacebookLeads(): Promise<FacebookLeadsResult> {
  ...
  if (!token || !locationId) {
    return {
      ...empty,
      error: "GHL_PRIVATE_INTEGRATION_TOKEN or GHL_LOCATION_ID is not set",
    };
  }
  ...
  while (contacts.length < MAX_CONTACTS) {
    ...
    try {
      ...
      if (contacts.length === 0) {
        return { ...empty, error: err instanceof Error ? err.message : "Failed to reach Apex Applications' GHL" };
      }
